g++ `pkg-config --cflags opencv` t.cpp `pkg-config --libs opencv` -lpthread -o exec.out
...
while [  $tam_kernel -lt 70 ]; do
  ...
  echo "" >> datos.txt
  echo "p_720p_kernel_"$tam_kernel" = [" >> datos.txt
  n_hilos=0
...
while [  $tam_kernel -lt 70 ]; do
  echo "procesando..." $tam_kernel
  echo "" >> datos.txt
  echo "p_1080p_kernel_"$tam_kernel" = [" >> datos.txt
  n_hilos=0
  while [  $n_hilos -lt 12 ]; do
    let n_hilos=n_hilos+1
...
while [  $tam_kernel -lt 70 ]; do
  echo "procesando..." $tam_kernel
  echo "" >> datos.txt
  echo "p_4k_kernel_"$tam_kernel" = [" >> datos.txt
  n_hilos=0
  while [  $n_hilos -lt 12 ]; do
    let n_hilos=n_hilos+1
